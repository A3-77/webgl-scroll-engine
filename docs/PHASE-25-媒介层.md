...
### 1.1 ⚠️ 后续更正：效果 ≠ 滤镜，运动才是另一半（PHASE 26）

这一章交付之后用户又补了一句，把方向掰了回来：

> "我不是让你去换滤镜  你懂吗你看看他们的动画顺滑又有创意"

**这一章的判断对了一半。** "差距不在效果的多少，在效果的种类"是对的 ——
媒介层确实是两个站点的共有物，也确实让画面从"照片"变成"印刷品"。
但它只覆盖了**画面长什么样**，完全没有碰**画面怎么动**。

参考站点看起来高级，一半靠材质，另一半靠**镜头怎么动**。
根因是相机缺一个 `target` —— 改造前镜头永远朝 −Z 看，只能平移一张图，
所以无论怎么加缓动/阻尼/视差，读起来都是"图片转场"。
详见 **[PHASE-26-镜头语言.md](./PHASE-26-镜头语言.md)**。

> **教训（写给下一次）**：遇到"他们的效果更好"，先问清楚是
> **画面长什么样** 还是 **画面怎么动**。这两个方向的实现没有任何重叠。
...
## 7.8 ★★ 剩下那段"观感差距"是素材的，不是引擎的 —— 用扫描证明

分级补上之后画面确实印出来了，但和参考站点的成品**并排**看，仍然差一口气：
他们的版画是**双峰**的（大片实黑 + 大片纸白），我这边整幅都泡在中间调里。

这不是"感觉"，是可以量的。对两边各算一次亮度分布：

| | 近实黑 0~0.10 | 暗 0.10~0.25 | **中间调 0.25~0.75** | 亮 0.75~0.90 | 近纸白 0.90~1 | 标准差 |
|---|---|---|---|---|---|---|
| 参考 `06-newsprint.png` | 2.7% | 15.4% | **15.0%** | 37.8% | 29.1% | **0.300** |
| 参考 `07-screentone.png` | 43.6% | 3.7% | **17.0%** | 35.4% | 0.3% | 0.344 |
| 参考 `08-popprint.png` | 41.5% | 11.9% | **35.7%** | 10.6% | 0.2% | 0.263 |
| **本引擎 `scene01`** | 2.2% | 12.7% | **69.4%** | 13.9% | 1.8% | **0.220** |

**中间调 69.4% vs 参考的 15~36%** —— 这就是"发糊"的量化定义。
不是网点没出来（网点很清楚），是**色调没有铺开**，点径变化范围太小。

### 先排除了"分级没调够"这个假设

把 shader 的链路（单色化 → 分级 → 屏幕空间网点栅格 → 纸色）在 numpy 里逐行复刻，
在**真实素材**上扫了 20 组参数：

- **抬 `blackPoint` 0.28 → 0.55**：中间调占比**升高**（62.3% → 81.4%）。
  它把"亮"整体压进"中间调"，不是拉开。直觉在这里是错的。
- **加 `contrast`**：中间调下降，但代价是**画面更亮**（中位 0.619 → 0.766），
  不是更"厚" —— 因为素材中位 0.764 在 S 曲线支点 0.5 之上，加对比只会把它推得更亮。
- **把 S 曲线支点从 0.5 移到画面中位**（0.60 / 0.70 / 0.75 / 0.80 × 对比 0.30/0.50/0.70）：
  **也无效**，标准差始终卡在 0.19~0.21。

20 组里最好的一档标准差只有 **0.215**，参考是 0.263~0.344。

### 结论

**源素材本身的亮度标准差只有 0.132**（均值 0.741、中位 0.764 —— 一组高调浅色油画）。
网点屏能把 0.132 拉伸到约 0.21，但**再也上不去了** —— 实黑占比无论怎么调都停在 0.9%。

参考站点不需要拉伸、也不需要陡分级，是因为它们的场景是**美术指导过的**：
深色背景 + 大块平涂色块，本身就跨满色阶。**它们的双峰来自场景里的实黑色块，
不是来自 shader。**

→ 所以这一段差距**不该由引擎去补**。硬补（猛拉对比）只会把画面推亮、推薄。
正确的做法是换一组有色调范围的素材，或者接受"高调照片印出来就是高调"。

### 想要那种海报感，旋钮是 `dither`，不是 `contrast`

引擎里本来就有有序抖动（色阶量化）。**量化到 N 级必然产生双峰** ——
这是唯一能把高调素材变成"海报"的正当手段。实测：

| 配置 | 中间调 | 近纸白 | 标准差 |
|---|---|---|---|
| `dither 0`（cats 现状） | 62.3% | 0.0% | 0.197 |
| `dither 1.0 levels 3` | 76.3% | 21.3% | 0.235 |
| `dither 1.0 levels 3 contrast 0.50` | 66.6% | **30.8%** | **0.241** |

最后一档的**近纸白 30.8% / 标准差 0.241**，已经落在 `06-newsprint`
（29.1% / 0.300）的同一区间里了。

**但 cats 包默认 `dither: 0`，是有意的**：那组素材是油画，
用户要的是"一张被印在纸上的油画"，不是"一张被做成海报的油画"。
`dither` 留着当旋钮 —— 谁想要海报感，谁自己拧。
